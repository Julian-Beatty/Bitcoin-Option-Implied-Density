...
# from skfda.preprocessing.smoothing.validation import akaike_information_criterion
...
class OptionMarket:
    ...
    def estimate_option_pdfs(self,method,argument_dict):
        ...
        if method=="local_poly":
            date_group.apply(lambda x: result_dict.update(local_poly(x,argument_dict)))
        if method=="local_smoother":
            date_group.apply(lambda x: result_dict.update(LL_smoother(x,argument_dict)))
...
def tail_extrapolation(strikes,iv):
    ...
    extrapolation_strikes=np.arange(strikes[-1]+average_step,strikes[-1]+13*average_step,average_step)
    ...
    extrapolation_strikes_left=np.arange(max(strikes[0]-13*average_step,0.5),strikes[0]-average_step,average_step)
...
def local_poly(option_df,argument_dict,interpolate_large = True):
    """
    Description of function: This function takes in an option dataframe, which contains the option quotes on a particular day, with a specific maturity.
    It will fit a N order polynomial to this IV curve to smooth out the implied volatilities, then convert them to calls, numerically differentiate twice and save the plots.
    Do not change the inputs of this function, and do not change the output.
    ...
    #parse arguments
    #argument_dict={"folder": "statsmodels","cv":"cv_ls"}
    cv=argument_dict["cv"]
    foldername=argument_dict["folder"]
    ...
    interpolated_strikes = np.linspace(min(strikes), max(strikes), 2000).reshape(-1)  # Fine grid of strikes for smooth curve
    ...
    ##Define interpolation grid
    # Evaluate the polynomial at the interpolated strike prices
    kernel_obj = KernelReg(endog=iv, exog=strikes, reg_type="ll", var_type='c',bw="cv_ml")
    interpolated_iv,_=kernel_obj.fit(interpolated_strikes)
    ...
    if plotting==True:
        ...
        if synthetic_used:
            ax1.scatter(synthetic_strikes, synthetic_ivs, color="orange",label="Synthetic IV")
        ax1.plot(interpolated_strikes, interpolated_iv,label="statsmodels IV")
        ax1.set_title(f"statsmodels on {date} to {exdate}")
        ax1.set_xlabel("Strikes")
        ax1.set_ylabel("Implied Volatility")
        ax1.legend()
        ax1.set_title("IV curve")
...
def LL_smoother(option_df,argument_dict,interpolate_large=True):
    """
    Depcrated from SCIKITFDA DO NOT USE.
    ...
    # x = np.linspace(0, 10, 15)  # Original data points
    # y = np.sin(x) + 0.1 * np.random.randn(15)  # Function with noise
    
    x=strikes.reshape(-1)
    y=iv
    # Convert data to FDataGrid format
    fd = FDataGrid(data_matrix=y.reshape(1, -1), grid_points=[x])  # Fixed shape
    
    
    ####
    strike_diffs = np.diff(strikes.reshape(-1))
    bandwidth = np.linspace(np.min(strike_diffs), np.max(strike_diffs), num=200)
    ...
    nw.fit(fd)
    best_bandwidth = nw.best_params_['kernel_estimator__bandwidth']
    print(best_bandwidth)
    
    #####
    # Define kernel smoother with a chosen kernel and bandwidth
    
    # Apply smoothing (interpolation) to the data
    fd_smoothed = nw.transform(fd)
    
    # Define a finer grid for interpolation
    
    interpolated_strikes = np.linspace(min(strikes), max(strikes),2000).reshape(-1)  
    
    # Create a new FDataGrid for the finer grid with placeholder values
    finer_fd = FDataGrid(
        data_matrix=np.full((1, len(interpolated_strikes)), np.nan),  # Placeholder values to match shape
        grid_points=[interpolated_strikes]
    )
    
    # Transform the finer grid (interpolation)
    fd_interpolated = fd_smoothed.evaluate(interpolated_strikes).squeeze()
    interpolated_iv = np.array(fd_smoothed.evaluate(interpolated_strikes)).squeeze()
    ...
    if plotting==True:
        ...
        if synthetic_used:
            ax1.scatter(synthetic_strikes, synthetic_ivs, color="orange",label="Synthetic IV")
        ax1.plot(interpolated_strikes, interpolated_iv,label="LL IV")
        ax1.set_title(f"scifda LL on {date} to {exdate}")
        ax1.set_xlabel("Strikes")
        ax1.set_ylabel("Implied Volatility")
        ax1.legend()
        ax1.set_title("IV curve")
        ...
        ax3.plot(pdf_strike,cdf)
        ax3.set_title("CDF")
        fig.suptitle(f"PDF for {date} to {exdate}", fontsize=16)
