...
def knn(option_df,argument_dict,interpolate_large = True):
    """
    ...
    # Create an array of neighbors for GridSearchCV
    n_neighbors = np.array(range(1, len(original_strikes)))


    # Set up GridSearchCV with Kernel Regression using KNeighborsHatMatrix
    knn = GridSearchCV(
        KernelRegression(kernel_estimator=KNeighborsHatMatrix()),
        param_grid={'kernel_estimator__n_neighbors': n_neighbors})

    # Fit the model
    knn.fit(combined_strikes,combined_iv)  
    ...
    interpolated_iv = knn.predict(interpolated_strikes)
    ...
    ax1.set_title(f"KNN interpolation on {date} to {exdate}")
